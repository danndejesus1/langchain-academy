...
def convert_json_to_csv(json_file_path: str, output_dir: str = "."):
    """Convert JSON file to separate CSV files using pandas"""
    ...
    for key, value in data.items():
        if isinstance(value, list) and len(value) > 0:
            ...
            # Save to CSV
            csv_filename = output_path / f"{key}.csv"
            df.to_csv(csv_filename, index=False)
            created_files.append(str(csv_filename))
            print(f"Created {csv_filename} with {len(df)} rows")
...
Make the data realistic with proper weights, quantities, and descriptions."""
    ...
    try:
        print("\nConverting JSON to CSV files using pandas...")
        converted_files = convert_json_to_csv(file_path)
